...
const RENDER_URL = 'https://veernxt-profiling-engine.onrender.com/api/recommend';
...
(async () => {
  ...
  // Pick 3 diverse profiles for the Render test to save time/resources
  const testIndices = [1, 5, 10]; // Amit, Mohit, Pawan
  const profilesToTest = testIndices.map(idx => {
    const row = parseCSVLine(lines[idx]);
    return mapCSVToProfile(row);
  }).filter(Boolean);

  console.log(`\n>>> TESTING RENDER ENGINE: ${RENDER_URL} <<<\n`);
  console.log(`(Note: This might take a minute if the instance is spinning up...)\n`);

  for (const profile of profilesToTest) {
    console.log(`----------------------------------------------------`);
    console.log(` SENDING: ${profile.fullName} | ${profile.armCorpsTrade} | ${profile.highestQualification}`);
    
    try {
      const startTime = Date.now();
      const response = await axios.post(`${RENDER_URL}?live=false&topN=3`, profile, {
      ...
      if (res.ok) {
        console.log(` SUCCESS (${endTime - startTime}ms) | Eligible: ${res.totalEligible}`);
        res.recommendations.forEach(r => {
          console.log(`   [${r.score}] ${r.career_track} - ${r.exam_name}`);
        });
      } else {
        console.error(` FAILED:`, res.errors || res.error);
      }
    } catch (e) {
      console.error(` ERROR calling Render:`, e.response ? e.response.data : e.message);
    }
  }
